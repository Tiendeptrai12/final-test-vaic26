...
def rank_products(retrieval: FilterResult, state, top_n: int = DEFAULT_TOP_N) -> RankingResult:
    ...
    # Bước RERANK (API-only, generic mọi category) — chèn TRƯỚC khi cắt top_n
    # để reranker thấy toàn bộ candidate pool. Fallback nội bộ: nếu tắt/lỗi/
    # timeout thì trả lại đúng thứ tự rule-based ở trên (không phá pipeline).
    from app.ranking.reranker import build_query_text, rerank_results

    scored = rerank_results(build_query_text(state), scored)
