...
| **C6 Compare / Evaluate** | ≥2 SP; 1 SP + nhu cầu; current vs upgrade; 2 item compatibility. Mode: contextual comparison, product-fit, compatibility, incremental-upgrade. | `comparison.compare` + `evaluation.evaluate_fit/compatibility/upgrade` | ✅ |
...
**Build status: toàn bộ C0–C10 đã có module.** (phone ranking ✅ · comparison ✅ · accessory ✅
· requery ✅). Còn lại = **wiring** các capability qua Router vào `nlu.build_chat_response`
(C9 vẫn UI-only theo scope advisor).
